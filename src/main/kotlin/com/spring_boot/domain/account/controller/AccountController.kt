...
class AccountController {
    ...
    @GetMapping("{id}")
    fun read(@PathVariable id: AccountId) = AccountRepository.findById(id)

    @ApiOperation(value = "Update a Specific Account", notes = "update a specific Account by request params", response = Account::class)
    @PostMapping("{id}")
    fun update(@PathVariable id: AccountId, @RequestParam params: Map<String, String>): Account {
        val requestParams = RequestParams(params)
    ...
    @DeleteMapping("{id}")
    fun delete(@PathVariable id: AccountId) {
